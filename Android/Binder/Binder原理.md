...
|    文件共享     |                           简单易用                           |        不适合高并发环境，并且无法做到进程间的实时通讯        |  无并发访问的情况，交换简单的数据，实时性不高的情况  |
|      aidl       |          功能强大，支持一对多并发通信，支持实时通信          |                使用稍复杂，需要处理好线程同步                |               一对多通信且且有RPC需求                |
|    Messenger    |          功能一般，支持一对多串行通信，支持实时通信          | 不能处理好高并发的情况，不支持RPC，数据通过message进行传输，因此只能传输bundle支持的数据类型 | 低并发一对多即时通讯无RPC需求，或者无法返回结构的RPC |
| ContentProvider | 在数据源访问方面功能强大，支持一对多并发数据共享，可通过call方法扩展其它操作。 |        可以理解受约束的AIDL，主要提供数据源的CRUD操作        |                 一对多进程间数据共享                 |
|     Socker      |  功能强大，可以通过网络传输字节流，支持一对多并发实时通信。  |            实现细节稍微有点繁琐，不支持直接的RPC             |                     网络数据交互                     |
...
Binder是Android中一种跨进程通信的方式。从Android FrameWork角度来说，Binder是ServiceManager连接各种Manager和相应ManagerService的桥梁。从Android应用层的角度来说， BInder是客户端和服务端进行通信的媒介。

**Binder线程池最大数为16，超过的请求会被阻塞。在进程间通信时处理并发时，如`ContentProvider`的CRUD（增删改查）操作方法最多有16个线程同时工作。**
...
![binder_arch](http://gityuan.com/images/binder/java_binder/java_binder.jpg)
...
![img](https://pic1.zhimg.com/80/v2-2160e70ca6e813ff28c34eec5ae8209c_1440w.jpg)
...
![img](https://pic3.zhimg.com/80/v2-30dce36be4e6617596b5fab96ef904c6_1440w.jpg)
...
![img](https://pic3.zhimg.com/80/v2-38e2ea1d22660b237e17d2a7f298f3d6_1440w.jpg)
...
![img](https://pic2.zhimg.com/80/v2-3c719337413b9c5c4ad0b6c6b8eb0291_1440w.jpg)
...
![img](https://pic3.zhimg.com/80/v2-729b3444cd784d882215a24067893d0e_1440w.jpg)
...
![img](https://pic4.zhimg.com/80/v2-7c68928e26f5b96b8b3471ebb1927107_1440w.jpg)
...
![img](https://pic4.zhimg.com/80/v2-67854cdf14d07a6a4acf9d675354e1ff_1440w.jpg)
...
![img](https://pic2.zhimg.com/80/v2-13361906ecda16e36a3b9cbe3d38cbc1_1440w.jpg)
...
![img](https://pic2.zhimg.com/80/v2-7ca457119bd700a5acf7f69bb0c07e51_1440w.jpg)
...
![binder_proc_relation](http://gityuan.com/images/binder/summary/binder_proc_relation.png)
