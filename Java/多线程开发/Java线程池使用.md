...
- shutdown原理：将线程池状态设置成SHUTDOWN状态，然后中断所有没有正在执行任务的线程(**关闭线程池，已提交的任务继续执行，不接受继续提交新任务**)。
- shutdownNow原理：将线程池的状态设置成STOP状态，然后中断所有任务(**包括正在执行的**)的线程，并返回等待执行任务的列表(**关闭线程池，尝试停止正在执行的所有任务，不接受继续提交新任务**)。
...
![thread-pool](http://gityuan.com/images/android-process/thread-pool.jpg)
